...
public class Task6Test {
    @Test
    @DisplayName("Самые тяжёлые каждого вида")
    void heaviest() {
        // given
        ...
        // when
        Map<Animal.Type, Animal> answer = Task6.heaviestOfEachSpecies(test);

        // then
        assertThat(answer.get(Animal.Type.BIRD).name()).isEqualTo("Kovalski");
        assertThat(answer.get(Animal.Type.CAT).name()).isEqualTo("Garfield");
        assertThat(answer.get(Animal.Type.FISH).name()).isEqualTo("Freddi");
    }
}
