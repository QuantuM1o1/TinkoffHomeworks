...
public class Task16Test {
    @Test
    @DisplayName("Сортировка по виду, полу, имени")
    void sortingSpeciesSexName() {
        // given
        ...
        // when
        List<Animal> answer = Task16.speciesSexName(test);

        // then
        assertThat(answer.get(0).name()).isEqualTo("Alex");
        assertThat(answer.get(1).name()).isEqualTo("Cheshire Cat");
        assertThat(answer.get(2).name()).isEqualTo("Garfield");
        assertThat(answer.get(3).name()).isEqualTo("Bastet");
        assertThat(answer.get(4).name()).isEqualTo("Mrs. Norris");
        assertThat(answer.get(5).name()).isEqualTo("Hachiko");
    }
}
